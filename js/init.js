...
function arlo_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		if(name===''||email===''||message===''){
			...
			jQuery.post("https://formspree.io/f/xzbyyedv",{ ajax_name: name, ajax_email: email, ajax_message:message, ajax_subject: subject}, function(data) {
...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["电子爱好者electronics enthusiast", "普通C程序员ordinary C programmer", "正在学习的人the one who is learning."],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
